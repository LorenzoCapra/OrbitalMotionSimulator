...
class ITVIM:
    ...
    def calc_traj(self):
        ...
        v0_sc, v1_sc = Tools.lamberts_universal_variables(
            state0[:3], state1[:3], tof, mu=mu, tm=tm)
        ...
        self.seq[0]['v_infinity'] = norm(v0_sc - state0[3:])

        self.seq[1]['state_sc_arrive'] = np.concatenate(
            (state1[:3], v1_sc))
        self.seq[1]['v_infinity'] = norm(v1_sc - state1[3:])
        ...
        for n in range(1, self.n_steps - 1):
            ...
            tof, v_sc_depart, v_sc_arrive = Tools.vinfinity_match(
            ...
            seq0['tof'] = tof
            seq0['tof_days'] = tof * Tools.sec2day
            seq0['turn_angle'] = Tools.vecs2angle(vinf_i, vinf_o) / 2.0
            seq0['periapsis'] = Tools.calc_close_approach(
                seq0['turn_angle'] * Tools.d2r,
    ...
    def print_summary(self):
        ...
        for n in range(self.n_steps):
            tof_days = self.seq[n]['tof'] * Tools.sec2day
            print(f'Segment {n}:')
    ...
    def plot_orbits(self, args={}):
        ...
        for n in range(self.n_steps - 1):
            ...
            sc = OrbitPropagator(sc_config['orbit_state'], sc_config['tspan'],
                                 sc_config['dt'], et0=sc_config['et0'],
                                 frame=sc_config['frame'], cb=sc_config['cb'],
                                 )
